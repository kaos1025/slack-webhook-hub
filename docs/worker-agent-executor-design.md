...
- `gemini-reviewer` supports two invocation modes for independent read-only review artifacts. Default is authenticated Gemini CLI via `GEMINI_REVIEW_COMMAND_JSON` / `route_snapshot.review.commandJson` to reuse CLI auth and avoid API-key token billing. Direct Gemini API is explicit opt-in only via `GEMINI_REVIEW_INVOCATION=api` or `route_snapshot.review.invocation="api"`. The default CLI command includes `--approval-mode plan` and `--skip-trust` for headless VPS runs.
